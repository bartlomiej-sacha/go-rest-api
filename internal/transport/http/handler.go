...
// Handler - storespointer to our comments service
type Handler struct {
	Router  *mux.Router
...
// write to a header may occur only once?
...
func (h *Handler) GetComment(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("Got getComment/{ID} request")
	...
	if err != nil {
		...
		sendErrorResponse(w, "Error Retreving Comment By ID", err)
		return
	}
...
func (h *Handler) PostComment(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		sendErrorResponse(w, "Failed post new comment", err)
	}
	if err := json.NewEncoder(w).Encode(comment); err != nil {
		panic(err)
	}
